...
- **Upgrade manifest and `hs upgrade` (P5, pulled by need 2026-09-10)**
  - Optional registry `install:` block (`InstallSpec`: `method`, `source`, `pin`, `version_cmd`, `asset`, `binary`) on `Service`, validated against a closed method enum (`github-release`, `xcaddy`, `source-go`, `npm-global`, `opencode`, `hermes-pinned`, `brew`); `source` always required, `asset`/`binary` required only for `github-release`. Carried unmodified into `catalog.json` (see `docs/SERVICE_INTERFACE.md` §4d). `profiles/default/services.yaml`'s pocket-id, tinyauth, caddy, opencode, openchamber, and hermes examples carry commented `install:` blocks with realistic values; a real pin is deployment state and lives in the owner's profile.
  - `hs upgrade status` (`portable/home-stack/scripts/lib/upgrade.sh`) reads every `install:`-tracked service from `catalog.json`, runs `version_cmd` for the live version, looks up the latest upstream release per method (`gh`, `npm`, `brew`), and classifies drift as `ok`/`behind`/`ahead-of-pin`/`unknown`; `--check` exits 1 if anything is `behind`.
  - `hs upgrade <service> [--to <version>] [--dry-run]` runs one reviewed procedure per method: fully automated for `github-release` (Pocket ID — download, checksum-verify when published, swap with `.prev`, verify `version_cmd`, restart, health-check, auto-revert), `source-go` (tinyauth — clone/build/swap, same health-check/revert), `npm-global` (OpenChamber), and `opencode`. `xcaddy` (Caddy) builds and stages but refuses to restart the system LaunchDaemon without `sudo`, printing the exact command instead of half-completing. `hermes-pinned` requires a full 40-character commit SHA via `--to` (never a tag) and `--yes` (it mutates `~/.hermes`), then verifies `/api/status` still reports `auth_required: true`. `brew` is status-only. Every apply appends one JSON line to `$HOME_STACK_CONFIG_DIR/upgrade-log.jsonl`.
  - `hs doctor` gains a pin-drift check: for each `install:` service with a `pin`, compares it to the live version and warns (never fails) on a mismatch.
  - Tests: `tests/hs-upgrade.test.sh` (Linux-safe, stubs `gh`/`npm`/`curl`/`install` via PATH and overridable `HOME_STACK_UPGRADE_*` variables) covers drift classification, `--check` exit codes, a github-release dry-run's resolved asset URL, a simulated successful apply (backup, install, log line), and a simulated failed health check's auto-revert; `hermes-pinned` refusing a tag. Go: `Validate` coverage for the method enum/required fields, and a new golden fixture (`tests/fixtures/profile-epsilon`) proving `install:` round-trips through the catalog while every pre-existing golden stays byte-identical.
